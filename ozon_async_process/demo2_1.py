    ...
    def searchResultsV2(self, cookies, ua, headers):
        try:
            response = self.session.get(
                self.url, cookies=cookies, headers=headers, impersonate=ua, timeout=30)
            ...
            return None
def download_image(url, headers, index, ws):
    try:
        if url:
            ...
            with requests.Session() as session:
                ...
                openpyxl_img = OpenpyxlImage(BytesIO(img_byte_arr.getvalue()))
                cell = 'A' + str(index)
...
if __name__ == '__main__':
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(download_image, Ozon_Spider(getattr(row, '商品链接')).searchResultsV2(cookies, ua, headers), headers, index, ws)
