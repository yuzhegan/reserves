...
class Pasewb():
    def __init__(self, datas, writer, search_text, page):
        self.datas = datas
        with open("./wildberries/deliv.js", "r", encoding="utf-8") as f:
            self.js = f.read()
    ...
    def GetdevileveyDay(self, time1, time2):
        ...
        parms = str(time)
        resutl = subprocess.run(["node",self.js, parms],capture_output=True, text=True)
        return resutl.stdout
    def GetImageurl(self, id):
        id = str(id)
        hosts = subprocess.run(["node", self.imgjs], capture_output=True, text=True).stdout
